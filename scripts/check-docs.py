    ...
    try:
        with sync_playwright() as playwright:
            ...
            try:
                ...
                page.goto(url + 'installation/#worker-requirements')
                expect(page.locator('article')).to_contain_text('kernel 5.4 or newer')
                expect(page.locator('article')).to_contain_text('Linux 5.4 support requires Sandweave 0.2.27')
                page.screenshot(path=str(output / 'installation-desktop.png'), animations='disabled')
                ...
                assert 'Storage(path="/data/sandbox-disks")' in copied and 'env.terminate()' in copied
                page.screenshot(path=str(output / 'writable-storage-desktop.png'), animations='disabled')
                page.goto(url + 'resources/#experimental-memory-sharing')
                section = page.locator('#experimental-memory-sharing')
                section.locator('xpath=following-sibling::div[1]').get_by_role('button', name='Copy to clipboard').click()
                copied = page.evaluate('navigator.clipboard.readText()')
                ...
                section = page.locator('#save-docker-storage')
                section.locator('xpath=following-sibling::div[1]').get_by_role('button', name='Copy to clipboard').click()
                copied = page.evaluate('navigator.clipboard.readText()')
                assert 'Sandbox(template="docker")' in copied and 'env.cache("docker-ready")' in copied
                page.screenshot(path=str(output / 'docker-storage-desktop.png'), animations='disabled')
                ...
                page.goto(url + 'resources/#experimental-memory-sharing')
                assert page.evaluate('document.documentElement.scrollWidth <= innerWidth')
                page.screenshot(path=str(output / 'memory-sharing-mobile.png'), animations='disabled')
                page.goto(url + 'snapshots/#save-docker-storage')
                assert page.evaluate('document.documentElement.scrollWidth <= innerWidth')
