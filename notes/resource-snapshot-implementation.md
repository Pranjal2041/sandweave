...
Host cgroups are root-owned and not delegated here. Host nice is per thread and does not give environment fairness: with four versus sixteen workers, nice 0/5 achieved 0.76:1 rather than the intended 3:1. The original controller accounted host CPU time across each registered process tree and paused guest execution through an idempotent private Sentry RPC. Host SIGSTOP was rejected after it interrupted systrap seccomp notification handling. Startup/restore is excluded until the sandbox reaches running state. Checkpointing temporarily suspends CPU control. As of 0.2.29, aggregate runtime accounting replaces process-tree sampling, broker pauses expire, and controller failure releases CPU policy while keeping the sandbox alive. See [CPU broker scaling](cpu-broker-scaling.md).
