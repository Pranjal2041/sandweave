...
@pytest.mark.parametrize('upgrade', ['kernel54', 'cpu_accounting', 'bridge_local_delivery', 'disk_storage'])
def test_upgrades_prepared_engine_without_rebuilding_guest(first_use, monkeypatch, tmp_path, upgrade):
    ...
    if upgrade in ('cpu_accounting', 'bridge_local_delivery', 'disk_storage'):
