...
from vr_frames import Frame, FrameRecorder, RecordedFrames
...
class VRStream:
    ...
    def suspend_attachment(self):
        """Release the lifecycle lease while keeping the resident XR processes and buffers.

        Used by a worker that serializes actions with pause/filesystem capture.
        This is not sufficient for a live GPU snapshot, which remains unsupported.
        """
        if self.lifecycle is not None:
            self.lifecycle.__exit__(None, None, None)
            self.lifecycle = None

    def resume_attachment(self):
        if self.lifecycle is None and not self.closed:
            self.lifecycle = io_lock(self.manager, self.name)
            self.lifecycle.__enter__()
            samples = []
            for _ in range(5):
    ...
    def abort_attachment(self):
        """Drop only owned host attachments before the owner discards the whole guest."""
        if self.closed:
            return
        self.suspend_attachment()
        for process in (self.bridge, self.monado):
