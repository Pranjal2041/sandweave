...
exec "$lab_root/scripts/native-control-exec.sh" bash -c '
set -euo pipefail
test ! -e /dev/kvm
id
uname -r
export DISPLAY=:98 XDG_RUNTIME_DIR=/session/runtime XDG_CONFIG_HOME=/session/config XDG_CACHE_HOME=/session/cache LP_NUM_THREADS=1 MOZ_ENABLE_WAYLAND=0
...
xdpyinfo >/dev/null
dbus-run-session -- /opt/firefox/firefox --no-remote --profile /session/firefox-profile http://127.0.0.1:28080/ > /session/firefox.log 2>&1
'
